...
import { UploadCloud, Package, ArrowRight, Loader2, Trash2, Calendar, Truck, Combine, X, Store, Users, Printer } from "lucide-react";
...
export default function ManagerDashboard() {
  ...
  const handleBulkDelete = async () => {
    const isConfirmed = window.confirm(`DANGER: Are you sure you want to delete all ${selectedPOs.length} selected POs?`);
    if (!isConfirmed) return;
    setUploading(true);
    ...
    for (let f = 0; f < files.length; f++) {
      ...
      try {
        ...
        if (activeTab === "DFI") {
          ...
          // PERFECT STANDARD CSV PARSER
          // Reads characters one by one, ignoring commas locked perfectly inside quotation marks
          const parseStandardCSV = (str: string) => {
            ...
            // Sanitize grid by throwing away blank rows
          ...
          // We safely target exactly the names inside your DFI raw CSV text!
          ...
          if (iPlu === -1 || iQty === -1) {
            throw new Error(`CRITICAL: Header row missing exact text 'Code - PLU' or 'Order Quantity'`);
          }
          ...
          for (let i = 1; i < grid.length; i++) {
            ...
            if (!barcode || barcode.length < 5) continue; // Skip random footers

            // Collect Globals from the very first valid data row
            ...
            // Multiply the exact column indexes
          ...
          if (parsedItems.length === 0) throw new Error("Could not find any Data Rows beneath headers.");
        }

        if (activeTab === "SOCIOLLA" || activeTab === "RESELLER") throw new Error("Module under construction.");

        // =============================================================
        // UNIVERSAL DATABASE SAVE LOGIC + BOX LPN GENERATION
        // =============================================================
        ...
        parsedItems.forEach(item => {
          for (let cartonNo = 1; cartonNo <= item.innerBoxes; cartonNo++) {
            ...
            boxesToInsert.push({
              po_id: poData.id, product_barcode: item.barcode, box_barcode: uniqueBoxBarcode,
              carton_number: cartonNo, total_cartons: item.innerBoxes, is_scanned: false
            });
            universalSequence++;
          }
        });
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-32"> 
      <div className="max-w-6xl mx-auto p-8">
          ...
          <div className={`border-2 border-dashed rounded-xl p-8 text-center transition tracking-wide relative ${activeTab === 'DFI' ? 'border-gray-300 bg-gray-50 hover:bg-gray-100' : activeTab === 'SOCIOLLA' ? 'border-pink-200 bg-pink-50/30 hover:bg-pink-50' : 'border-amber-200 bg-amber-50/30 hover:bg-amber-50'}`}>
            ...
            {uploading ? (
              <div className="flex flex-col items-center text-blue-600">
                ...
                <span className="font-semibold">Processing Data Streams...</span>
              </div>
            ) : (
              <div>
                <p className="font-semibold text-lg text-gray-700">Click or Drag <span className="font-bold">{activeTab}</span> Files Here</p>
                <p className="text-sm text-gray-500 mt-1">Targeting Standard DFI/Guardian CSV Format</p>
              </div>
            )}
          </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {pos.length === 0 ? (
              <div className="col-span-full p-8 border-2 border-dashed border-gray-200 rounded-xl text-center text-gray-500 bg-white">Queue is empty. Upload files to start.</div>
            ) : (
              pos.map((po: any) => {
                ...
                return (
                  <div key={po.id} className={`bg-white border-2 p-6 rounded-xl shadow-sm flex flex-col justify-between transition group relative cursor-default ${isSelected ? 'border-blue-500 bg-blue-50/30' : 'border-gray-200 hover:border-black'}`}>
                    ...
                    <div className="mt-6 flex flex-col gap-2">
                       <Link href={`/labels/${po.id}`}>
                         <button className="w-full bg-blue-50 text-blue-700 border border-blue-200 py-3 rounded-lg font-bold flex items-center justify-center gap-2 hover:bg-blue-100 transition"><Printer className="w-4 h-4" /> Print Labels</button>
                       </Link>
                       <Link href={`/pack/${po.id}`}>
                         <button className="w-full bg-black text-white py-3 rounded-lg font-bold flex items-center justify-center gap-2 hover:bg-gray-800 transition active:scale-[0.98]">Enter 2FA Mode <ArrowRight className="w-4 h-4" /></button>
                       </Link>
                    </div>
                  </div>
                );
      ...
      {selectedPOs.length > 0 && ( /* batch actions footer remains active */
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-white border border-gray-200 shadow-2xl rounded-2xl px-6 py-4 flex items-center justify-between gap-8 z-50 w-full max-w-2xl animate-fade-in-up">
          <div className="flex items-center gap-4">
            <div className="bg-blue-100 text-blue-800 w-10 h-10 rounded-full flex items-center justify-center font-black text-xl">{selectedPOs.length}</div>
            <div>
